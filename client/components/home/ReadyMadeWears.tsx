// Fixed ReadyMadeWears Component with Skeleton
"use client"

import React, { useState, useEffect, useRef } from 'react';
import { motion, useInView } from 'framer-motion';
import { getReadyMadeWears, SanityProduct } from '../../lib/sanity';
  ...
  useEffect(() => {
    if (isInView && !dataFetched) {
      const fetchData = async () => {
        setLoading(true);
        try {
          const data = await getReadyMadeWears(4);
          setProducts(data);
          setDataFetched(true);
        } catch (error) {
          console.error('Error fetching ready-made wears:', error);
        } finally {
          setLoading(false);
        }
      };
