// Fixed HandMadeShoes Component with Skeleton
"use client"

import React, { useState, useEffect, useRef } from 'react';
import { motion, useInView } from 'framer-motion';
import { getHandMadeShoes, SanityProduct } from '../../lib/sanity';
  ...
  useEffect(() => {
    if (isInView && !dataFetched) {
      const fetchData = async () => {
        setLoading(true);
        try {
          const data = await getHandMadeShoes(undefined, 4);
          setProducts(data);
          setDataFetched(true);
        } catch (error) {
